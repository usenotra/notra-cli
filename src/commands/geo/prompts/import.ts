...
export default class GeoPromptsImport extends NotraCommand {
  static override description = 'Bulk import GEO prompts from JSON or CSV.';
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(GeoPromptsImport);
    const input = await readTextFromFileOrStdin(
      flags['config-file'],
      'Expected prompt JSON or CSV via --config-file or piped on stdin.',
    );
    const body =
      flags.format === 'csv' ||
    ...
    const response = await this.geo().request(
      'POST',
      `/v1/projects/${encodeURIComponent(args.projectId)}/geo/prompts/import`,
      { body },
    );
