...
export default class GeoVisibilityTimeseries extends NotraCommand {
  static override description = 'Get daily GEO visibility mention counts by engine.';
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(GeoVisibilityTimeseries);
    const response = await this.geo().request(
      'GET',
      `/v1/projects/${encodeURIComponent(args.projectId)}/geo/visibility/timeseries`,
