...
export default class GeoVisibilityLanguageShare extends NotraCommand {
  static override description = 'Get GEO visibility mention rates by language.';
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(GeoVisibilityLanguageShare);
    const response = await this.geo().request(
      'GET',
      `/v1/projects/${encodeURIComponent(args.projectId)}/geo/visibility/language-share`,
